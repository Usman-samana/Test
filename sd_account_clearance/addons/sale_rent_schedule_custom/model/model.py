...
class SaleRentSchedule(models.Model):
    ...
    @api.model
    def update_partner_minli(self):

        srs = self.env['sale.rent.schedule'].search([('id', 'in', [14416,14417,14418,14419,14420,14421,14422,14423,14424,14425,14426,14427,14428,14429,14430,14431,14432,14433,14434,14435,14436,14437,14438,14439,14440,14441,14442,14443,14444,14445,14446,14447,14448,14449,14450,14451,14452,14453,14454,14455,14456])])
        for rec in srs:
            rec.partner_id = 916
    ...
    def create_invoice_auto(self):
        ...
        sale_rent = self.env['sale.rent.schedule'].search([('start_date','<=',datetime.now().date()),('inv','!=',True),('state','not in',['cancel'])])
        print(len(sale_rent))
